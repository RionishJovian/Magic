-- Sandphase 1 rewrote enforce_device_quota_trigger to skip sandbox routers via
-- NEW.connection_mode. That field only exists on router_connections. PL/pgSQL
-- resolves NEW.<col> against the firing table, so site (and controller) inserts
-- fail with: record "new" has no field "connection_mode".
-- Read the optional field through jsonb so the shared trigger works on all three tables.
...
BEGIN
  ...
  -- Use to_jsonb so sites/controllers (no connection_mode column) stay safe.
